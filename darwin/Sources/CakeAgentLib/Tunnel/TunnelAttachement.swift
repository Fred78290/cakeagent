...
typealias TunnelAttachements = [TunnelAttachement]

extension TunnelAttachements {
	public func validate() throws {
		try self.forEach { port in
...
extension TunnelAttachement.ForwardUnixDomainSocket {
	public func validate() -> Bool {
		if proto == .none || host.isEmpty || guest.isEmpty {
			return false
		}

		if self.host.utf8.count > 103 {
			return false
		}

		if self.guest.utf8.count > 103 {
			return false
		}
